...
- **混合內容** - 固定風險提醒 + 國際時事(精簡列表) / 指數焦點(emoji漲跌，含日經與上證) / 前一交易日台股收盤
...
UNSPLASH_ACCESS_KEY=你的Unsplash_Access_Key（可選，用於每日單字圖片功能）
```

**可選：設置 Unsplash API（用於每日單字圖片）**

如果想要每日單字功能包含精美圖片，請執行以下步驟：

1. 前往 [Unsplash Developers](https://unsplash.com/developers)
2. 註冊並創建新應用
3. 在應用頁面複製 "Access Key"
4. 將 Access Key 添加到 `.env` 文件中的 `UNSPLASH_ACCESS_KEY`

⚠️ **注意：** 如果不設置 Unsplash API，每日單字功能仍可正常使用，只是不會包含圖片。
...
#### `/daily-word setup` - 設置每日單字功能
設置每日單字功能的基本配置（僅管理員/權限角色）。

**參數:**
- `頻道` (必填) - 發送單字的頻道
- `時間` (可選) - 發送時間，格式 HH:mm，預設 09:00
- `難度` (可選) - JLPT 等級，預設全部等級
- `用戶` (可選) - 要提及的用戶

**範例:**
```
# 基本設置
/daily-word setup 頻道:#日語學習 時間:09:00 難度:N5

# 完整設置（包含提及用戶）
/daily-word setup 頻道:#study 時間:20:00 難度:全部等級 用戶:@學習小組
```

設置完成後會立即發送一個測試單字，確認配置正確。

#### `/daily-word send-now` - 立即發送單字
立即發送一個隨機單字，用於測試功能（僅管理員/權限角色）。
```
/daily-word send-now
```

#### `/daily-word status` - 查看狀態
查看當前每日單字功能的配置和統計資訊（所有人可用）。
```
/daily-word status
```
...
#### `/daily-word toggle` - 開啟/關閉
開啟或關閉每日單字功能（僅管理員/權限角色）。
```
/daily-word toggle
```

#### `/daily-word history` - 查看歷史
查看最近發送的 10 個單字（所有人可用）。
```
/daily-word history
```

#### `/daily-word reset-history` - 清空歷史
清空單字歷史記錄，單字會重新開始循環（僅管理員/權限角色）。
```
/daily-word reset-history
```

**⚠️ 注意：** 清空後已發送的單字可能會再次出現。

### 📈 股市提醒指令

#### `/stock-reminder setup` - 設置股市提醒功能
設置每日股市提醒的基本配置（僅管理員/權限角色）。
...
```
# 基本設置
/stock-reminder setup 頻道:#finance 時間:08:30

# 完整設置（包含提及用戶）
/stock-reminder setup 頻道:#market-watch 時間:08:30 用戶:@交易小組
```
...
**Q: 如何設置每日單字功能?**  
A: 使用 `/daily-word setup` 指令進行設置，設置完成後會立即發送一個測試單字。

**Q: 單字會重複嗎?**  
A: 系統會記錄最近 500 個已發送的單字，避免短期內重複。當所有單字都發送過後，會自動清空歷史記錄重新開始。

**Q: 如果沒有設置 Unsplash API 會怎樣?**  
A: 單字卡片仍會正常發送，只是不會包含圖片。功能完全可用。

**Q: 如何更改發送時間或難度等級?**  
A: 重新執行 `/daily-word setup` 指令即可更新配置。

**Q: 單字發送失敗了怎麼辦?**  
A: 系統會自動重試 3 次。如果仍失敗會靜默跳過，隔天會重新嘗試。可以使用 `/daily-word send-now` 手動發送。

**Q: 可以一天發送多個單字嗎?**  
A: 目前只支持每天定時發送一個單字，但可以使用 `/daily-word send-now` 手動發送額外的單字。

**Q: 如何暫時停止每日單字?**  
A: 使用 `/daily-word toggle` 指令可以快速開啟或關閉功能。
