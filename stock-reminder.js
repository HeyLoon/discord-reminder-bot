...
const FALLBACK_DIGEST = {
    international: [{ title: '暫無即時摘要，請留意主要央行與地緣政治消息。', link: null }],
    indices: [
        '🇺🇸 S&P 500      N/A  --',
        '🇺🇸 Dow Jones    N/A  --',
        '🇺🇸 Nasdaq 100   N/A  --',
...
function formatIndexLine(label, value, changeText, marketEmoji, changeValue) {
    ...
    const trendMark = Number.isFinite(changeValue) && changeValue > 0
        ? '🟢'
...
async function fetchRssItems(url, maxItems = INTERNATIONAL_ITEMS, retryCount = 0) {
    try {
        ...
        console.error(`[Stock Reminder] RSS 讀取失敗 (嘗試 ${retryCount + 1}/${MAX_RETRY}): ${url} - ${error.message}`);
...
async function fetchCategoryDigest(urls = [], fallbackItems = [], maxItems = INTERNATIONAL_ITEMS) {
    const items = [];
...
async function fetchStooqIndex(symbol, label, marketEmoji, retryCount = 0) {
    try {
        const response = await axios.get(`https://stooq.com/q/l/?s=${encodeURIComponent(symbol)}&f=sd2t2ohlcv&h&e=csv`, {
            timeout: 10000,
            responseType: 'text',
            headers: {
                'User-Agent': 'discord-reminder-bot/1.0'
        ...
        const change = Number.isFinite(parsed.open) ? parsed.close - parsed.open : null;
        ...
        console.error(`[Stock Reminder] 指數讀取失敗 (嘗試 ${retryCount + 1}/${MAX_RETRY}): ${label} - ${error.message}`);
...
async function fetchSinaIndex(symbol, label, marketEmoji, retryCount = 0) {
    try {
        ...
        const base = Number.isFinite(parsed.change) ? parsed.close - parsed.change : null;
        ...
        console.error(`[Stock Reminder] 新浪指數讀取失敗 (嘗試 ${retryCount + 1}/${MAX_RETRY}): ${label} - ${error.message}`);
...
async function fetchNikkeiIndex(retryCount = 0) {
    try {
        ...
        console.error(`[Stock Reminder] 日經指數讀取失敗 (嘗試 ${retryCount + 1}/${MAX_RETRY}): ${error.message}`);
...
function pickNearestTxNightContract(rows, latestDate) {
    const candidates = rows
        ...
        .sort((a, b) => Number(a['ContractMonth(Week)']) - Number(b['ContractMonth(Week)']));
...
async function fetchTaiwanNightIndex(retryCount = 0) {
    try {
        const response = await axios.get('https://openapi.taifex.com.tw/v1/DailyMarketReportFut', {
            timeout: 12000,
            headers: {
                'User-Agent': 'discord-reminder-bot/1.0'
            }
        ...
        const changeText = formatChange(change, Number.isFinite(open) ? open : null);

        return [
            formatIndexLine('台指夜盤', formatNumber(lastValue, 0), changeText, '🇹🇼', change)
        ];
    } catch (error) {
        console.error(`[Stock Reminder] 台指夜盤讀取失敗 (嘗試 ${retryCount + 1}/${MAX_RETRY}): ${error.message}`);
...
async function fetchTaiwanPreviousClose(retryCount = 0) {
    try {
        ...
        const signedChange = Number.isFinite(changePoint) ? changePoint * sign : null;
        const prevClose = Number.isFinite(close) && Number.isFinite(signedChange)
            ? close - signedChange
            : null;
        ...
        console.error(`[Stock Reminder] 台股收盤讀取失敗 (嘗試 ${retryCount + 1}/${MAX_RETRY}): ${error.message}`);
...
async function fetchFinanceDigest() {
    const [international, headlineIndices, twNightIndex, twPreviousClose] = await Promise.all([
        fetchCategoryDigest(FINANCE_FEEDS.international, FALLBACK_DIGEST.international, INTERNATIONAL_ITEMS),
...
function buildStockReminderEmbed(digest, timezone) {
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields(
            ...
            {
                name: '📊 指數焦點',
                value: ['```', ...digest.headlineIndices, ...digest.twNightIndex, '```'].join('\n')
            },
            {
    ...
    embed
        .setFooter({ text: `來源: 公開新聞 RSS / Stooq / 新浪 / TWSE / TAIFEX • ${currentTime}` })
        .setTimestamp();

    return embed;
}

async function sendStockReminder(client, guildId, channelId, config, isTest = false) {
    try {
        console.log(`[Stock Reminder] 開始為伺服器 ${guildId} 發送${isTest ? '測試' : ''}財經提醒...`);
        ...
        const embed = buildStockReminderEmbed(digest, config.timezone || DEFAULT_TIMEZONE);
        ...
        if (!isTest) {
            updateGuildStockConfig(guildId, {
                lastSentDate: moment().tz(config.timezone || DEFAULT_TIMEZONE).format('YYYY-MM-DD'),
...
async function checkStockReminderSchedules(client) {
    try {
        ...
        for (const [guildId, guildConfig] of Object.entries(config.guilds)) {
            if (!guildConfig.enabled || !guildConfig.channelId || !guildConfig.sendTime) {
                continue;
            }
            ...
            await sendStockReminder(client, guildId, guildConfig.channelId, guildConfig, false);
        }
    } catch (error) {
